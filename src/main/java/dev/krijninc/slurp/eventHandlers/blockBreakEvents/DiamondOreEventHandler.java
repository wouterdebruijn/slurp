    ...
    protected void sendMessage(Player trigger, DrunkEntry entry) {
        if (entry.getPlayer().equals(trigger.getUniqueId())) {
            if (entry.getSips() > 0 && entry.getShots() < 1)
                Slurp.sendMessage(trigger, ChatColor.GOLD + "You mined diamonds, now take " + entry.getSips() + " sips and " + entry.getShots() + " shots!");
            else if (entry.getSips() > 0)
                Slurp.sendMessage(trigger, ChatColor.GOLD + "You mined diamonds, now take " + entry.getSips() + " sips!");
            else
                Slurp.sendMessage(trigger, ChatColor.GOLD + "You mined diamonds, now take " + entry.getShots() + " shots!");

        } else {
            Player drinker = Slurp.getPlugin().getServer().getPlayer(entry.getPlayer());
            if (drinker == null) return;
            if (entry.getSips() > 0 && entry.getShots() < 1)
                Slurp.sendMessage(drinker, ChatColor.GOLD + trigger.getDisplayName() + " mined diamonds, now you take " + entry.getSips() + " sips and " + entry.getShots() + " shots!");
            else if (entry.getSips() > 0)
                Slurp.sendMessage(drinker, ChatColor.GOLD + trigger.getDisplayName() + " mined diamonds, now you take " + entry.getSips() + " sips!");
            else
                Slurp.sendMessage(drinker, ChatColor.GOLD + trigger.getDisplayName() + " mined diamonds, now you take " + entry.getShots() + " shots!");
        }
    }
